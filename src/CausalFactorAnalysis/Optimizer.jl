...
function minimum_variance_factor_weights(covariance, factor_exposures, target_exposures)
    ...
    size(exposure_array, 1) == n_assets || throw(
        ArgumentError("factor_exposures must have the same number of rows as covariance"),
    )
    n_factors = size(exposure_array, 2)
    n_factors > 0 || throw(ArgumentError("factor_exposures must contain a factor column"))
    n_factors <= n_assets ||
        throw(ArgumentError("factor_exposures cannot have more columns than rows"))
    length(target_array) == n_factors || throw(
        ArgumentError("target_exposures length must equal the number of factor columns"),
    )

    covariance_scale = maximum(abs, covariance_array)
    covariance_scale > 0.0 || throw(ArgumentError("covariance must be positive definite"))
    ...
    scaled_covariance = 0.5 .* scaled_covariance .+ 0.5 .* transpose(scaled_covariance)
    ...
    any((target_array .!= 0.0) .& (scaled_targets .== 0.0)) && throw(
        ArgumentError("factor constraints could not be represented at numerical precision"),
    )
    ...
    all(isfinite, whitened_exposures) ||
        throw(ArgumentError("factor_exposures produce non-finite whitened values"))
    ...
    all(isfinite, achieved_exposures) && all(residuals .<= tolerances) || throw(
        ArgumentError("factor constraints could not be resolved to numerical precision"),
    )
